/* EJECUTAR ESTO PRIMERO en DatPosAdmin para saber tu ccod_empresa */
USE DatPosAdmin;
GO
...
INSERT INTO Empresas(ccod_empresa, cdsc_empresa, cnum_tribu, cnombre_bd, cnombre_servidor, cdomicilio, cdoc, cnomser)
VALUES('EMP01', 'EMPRESA DEMO DATPOS', '20000000001', 'DatPos_EMP01', 'localhost', 'Av. Principal 123, Lima', 'RUC', 'localhost');
GO

IF NOT EXISTS (SELECT 1 FROM Usuarios WHERE ccod_usuario='ADMIN')
INSERT INTO Usuarios(ccod_usuario, cdsc_usuario, cpassw, id_rol, ccod_empresa, id_estado, ccod_tiend, ccod_almacen, ccod_caja, cperm_descn)
...
-- 1. Ver el código de empresa del usuario ADMIN
SELECT ccod_usuario, cdsc_usuario, ccod_empresa
FROM Usuarios
WHERE ccod_usuario = 'ADMIN';

-- 2. Ver todas las empresas registradas
SELECT ccod_empresa, cdsc_empresa, cnombre_bd
FROM Empresas;

-- El valor de ccod_empresa que aparezca es el que debes
-- reemplazar por 'EMP01' en el script 13 (seed data)
-- y en el script 03 (accesos del rol Administrador)
