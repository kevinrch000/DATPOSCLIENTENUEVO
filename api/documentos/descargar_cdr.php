<?php
/**
 * Endpoint: Descargar CDR (Constancia de Recepción de SUNAT)
 * GET /api/documentos/descargar_cdr?serie=B001&correlativo=62
 */
...
try {
    $bl = new DocumentosBL();
    $data = $bl->obtenerCdr($serie, $correlativo, $o);

    // Un ZIP (CDR) válido debe empezar con "PK" y no contener el texto de mock de prueba
    $esValido = ($data !== null && strlen($data) > 2 && substr($data, 0, 2) === 'PK' && strpos($data, 'Mock ZIP') === false);
    ...
    // Si no es un ZIP válido pero el comprobante existe, lo generamos en caliente
    if (!$esValido && $comprobanteDatos !== null) {
        $data = DocumentosHelper::generarCdr($comprobanteDatos['header']);
    }

    // Servir el CDR para descarga directa (es un archivo ZIP)
    $filename = 'R-' . $serie . '-' . $correlativo . '.zip';
    ...
    error_log("Error en descargar_cdr.php: " . $e->getMessage());
    header('Content-Type: application/json; charset=utf-8');
