<?php
/**
 * Endpoint: Descargar PDF de Comprobante
 * GET /api/documentos/descargar_pdf?serie=B001&correlativo=62
 */
...
try {
    $bl = new DocumentosBL();
    $data = $bl->obtenerPdf($serie, $correlativo, $o);

    // Un PDF válido debe empezar con "%PDF" y no contener el texto de mock de prueba
    $esValido = ($data !== null && strlen($data) > 4 && substr($data, 0, 4) === '%PDF' && strpos($data, 'Mock PDF') === false);
    ...
    // Si no es un PDF válido pero el comprobante existe, lo generamos en caliente
    if (!$esValido && $comprobanteDatos !== null) {
        $data = DocumentosHelper::generarPdf($comprobanteDatos['header'], $comprobanteDatos['details'], $comprobanteDatos['header']);
    }

    // Servir el PDF para descarga directa
    $filename = $serie . '-' . $correlativo . '.pdf';
    ...
    error_log("Error en descargar_pdf.php: " . $e->getMessage());
    header('Content-Type: application/json; charset=utf-8');
