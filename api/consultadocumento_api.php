...
switch ($m) {
    ...
    case 'ConsultaTributarioPrincipal':
        ...
        foreach ($rows as $f) {
            ...
            $pdf = $idDoc !== '' ? "<a href='#' class='btn-descargar-pdf' data-serie='{$f[3]}' data-correlativo='{$f[4]}' title='Descargar PDF'><i class='fa fa-file-pdf-o text-danger'></i></a>" : '';
            $xml = $idDoc !== '' ? "<a href='#' class='btn-descargar-xml' data-serie='{$f[3]}' data-correlativo='{$f[4]}' title='Descargar XML'><i class='fa fa-file-code-o text-warning'></i></a>" : '';
            $zip = $idDoc !== '' ? "<a href='#' class='btn-descargar-cdr' data-serie='{$f[3]}' data-correlativo='{$f[4]}' title='Descargar CDR'><i class='fa fa-file-archive-o text-info'></i></a>" : '';
            $lst[] = array(
    ...
    case 'ConsultarArticulosPricipal':
        ...
        // FIX: el JS usa '%%%' como señal de "Todos" (sin filtro).
        // El SP solo entiende '' como "sin filtro" → convertir %%% a ''.
        // También convertir '1'/'0' (Estado) al código que guarda la BD ('A'/'I').
        $fn_todos = function($v) { $v = trim($v ?? ''); return ($v === '%%%') ? '' : $v; };
        $codArt    = $fn_todos($data['CodArticulo'] ?? '');
        $nomArt    = $fn_todos($data['NomAticulo']  ?? '');
        $tipArt    = $fn_todos($data['TipArticulo'] ?? '');
        $tributos  = $fn_todos($data['Tributos']    ?? '');
        $familia   = $fn_todos($data['Familia']     ?? '');
        $uniMed    = $fn_todos($data['UniMedida']   ?? '');
        // Estado: el JS envia '%%%'=todos, '1'=Activo, '0'=Inactivo
        // La BD guarda 'A'/'I' en cstatus
        $estadoRaw = $fn_todos($data['Estado'] ?? '');
        if ($estadoRaw === '1') $estadoRaw = 'A';
        elseif ($estadoRaw === '0') $estadoRaw = 'I';
    ...
    case 'ConsultaArticulosMasVendidos':
        ...
        // FIX %%%: convertir '%%%' a '' (sin filtro) antes de pasar al SP
        $fn_mv = function($v){ $v = trim($v ?? ''); return ($v === '%%%') ? '' : $v; };
        $rows = Database::selectStoredTenant('webDatpos_ConsultaArticulosMasVendidos', array(
            '@ccod_tienda'   => $fn_mv($data['ccod_tienda']   ?? ''),
            '@ccod_articulo' => $fn_mv($data['ccod_articulo'] ?? ''),
            '@ccod_lin'      => $fn_mv($data['ccod_lin']      ?? ''),
    ...
    case 'ConsultaListPrecioPricipal':
        ...
        // FIX %%%: convertir '%%%' a '' (sin filtro) antes de pasar al SP
        $fn_lp = function($v){ $v = trim($v ?? ''); return ($v === '%%%') ? '' : $v; };
        $rows = Database::selectStoredTenant('webDatpos_ConsultaListPrecioPricipal', array(
            '@ccod_cblistpre'    => $fn_lp($data['ccod_cblistpre']    ?? ''),
            '@ccod_articulo'     => $fn_lp($data['ccod_articulo']     ?? ''),
            '@cdsc_articulo'     => $fn_lp($data['cdsc_articulo']     ?? ''),
            '@ccod_lin'          => $fn_lp($data['ccod_lin']          ?? ''),
            '@ccod_unidadmedida' => $fn_lp($data['ccod_unidadmedida'] ?? ''),
            '@CodCia'            => $o->ccod_empresa,
        ), $o);
        $lst = array();
        foreach ($rows as $f) {
    ...
    case 'ConsultaStockMinimoPrincipal':
        // FIX BUG 2.3.4: el JS de ConsultaStockMinimo1.js espera los campos:
        //   ccod_alm, ccod_articulo, cdsc_articulo, cdsc_lin, ncantidad, nstock_min, nstock_max
        // Usamos webDatpos_ConsultaStockMinimo (5 params) que devuelve:
        //   [0]ccod_articulo, [1]cdsc_articulo, [2]nstock_min, [3]ncantidad, [4]cdsc_alm
        // y construimos la respuesta con los nombres que espera el JS.
        $input = getJsonInput();
        $data  = $input['stockminimo'][0]  // llamada desde ConsultaStockMinimo1.js
              ?? $input['Consultar'][0]    // llamada legacy
              ?? array();
        // FIX %%%: el JS usa '%%%' como "Todos" — el SP solo entiende '' como sin filtro.
        $fn_p = function($v){ $v = trim($v ?? ''); return ($v === '%%%') ? '' : $v; };
        $ccod_alm     = $fn_p($data['ccod_alm']     ?? '');
        $cdsc_lin     = $fn_p($data['cdsc_lin']     ?? '');
        $ccod_articulo= $fn_p($data['ccod_articulo'] ?? ($data['id_articulo'] ?? ''));
        $rows = Database::selectStoredTenant('webDatpos_ConsultaStockMinimo', array(
            '@ccod_cia'      => $o->ccod_empresa,
            '@ccod_alm'      => $ccod_alm,
            '@ccod_lin'      => $cdsc_lin,
        ...
        // SP del FIX_55 devuelve 7 cols:
        // [0]ccod_alm [1]ccod_articulo [2]cdsc_articulo [3]cdsc_lin
        // [4]ncantidad [5]nstock_min [6]nstock_max
        $lst = array();
        foreach ($rows as $f) {
            $lst[] = array(
                'ccod_alm'      => strval($f[0] ?? $ccod_alm),
                'ccod_articulo' => strval($f[1] ?? ''),
                'cdsc_articulo' => strval($f[2] ?? ''),
                'cdsc_lin'      => strval($f[3] ?? $cdsc_lin),
                'ncantidad'     => strval($f[4] ?? '0'),
                'nstock_min'    => strval($f[5] ?? '0'),
                'nstock_max'    => strval($f[6] ?? '0'),
            );
        }
        jsonResponse(array('d' => $lst));
        break;
