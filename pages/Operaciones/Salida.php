<?php
/**
 * DatPOS - Salidas directas
 * Reemplaza: Operaciones/Salida.aspx + Salida.aspx.vb
 */
...
$pageTitle = 'Salida de Almacén | DATPOS';
$pageScript = 'Salida.js';
$pageScriptPatch = 'operaciones_patch.js';
$showCrudButtons = true;

// Cargar tipos de operación de tipo Salida (server-side, como Page_Load del VB)
$DA = new DASalida();
$tiposOper = $DA->ConsultarTiposOperacionActivosSalidas($o->ccod_empresa, $o);
...
<script>window.OPERACION_ASPX = 'Salida.aspx';</script>
...
<div class="c-content-center modern-page">
    ...
    <div class="tab-content">
        ...
        <div id="Datos" class="tab-pane in active">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group"><label class="col-form-label">Stock disponible</label><input type="text" class="form-control" id="tb_cantActual" readonly onclick="ObtenerNombreColumna(this)"/></div>
                            ...
                            <div class="form-group"><label class="col-form-label">Costo</label><input type="number" min="0" class="form-control" id="tb_costo" readonly onclick="ObtenerNombreColumna(this)"/></div>
                        </div>
            ...
            <!-- Modal Artículos (filtrados por almacén con stock>0) -->
            <div class="modal fade" id="modalArticulos" tabindex="-1" role="dialog" aria-labelledby="modalArticuloLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="modalArticuloLabel">Artículos disponibles</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                        </div>
                        <div class="modal-body">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group"><label class="col-form-label">Costo</label><input type="number" min="0" class="form-control" id="tb_costo_editar" readonly onclick="ObtenerNombreColumna(this)"/></div>
                        </div>
                        <div class="modal-footer">
                        ...
                        <ul class="nav navbar-nav">
                            <li class="dropdown">
                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    <li><a href="#" onclick="FilterTipo('VENT');">Venta / Salida</a></li>
                                    <li><a href="#" onclick="FilterTipo('AJER');">Ajuste de Egreso</a></li>
                                </ul>
                            </li>
                        </ul>
                    </div>
...
<script>
// Salida.js no setea tb_um (no existia en el ASPX original).
// Cuando se cambia tb_articulo (lo setea ValidarArticuloAlmacenSalida o PasarArticulo),
// hacemos un fetch ligero a la API para llenar la unidad de medida.
$(document).on('change', '#tb_cod', function () {
    var cod = $('#tb_cod').val();
    var alm = $('#ddl_almacen').val();
    if (!cod || !alm) return;
    $.ajax({
        type: 'POST',
        url: 'Salida.aspx/ValidarArticuloAlmacenSalida',
        data: JSON.stringify({ ccod_articulo: cod, ccod_alm: alm }),
        contentType: 'application/json; charset=utf-8',
        dataType: 'json',
        success: function (r) {
            if (r && r.d && r.d.length) {
                $('#tb_um').val(r.d[0].uni_medi || '');
            } else {
                $('#tb_um').val('');
            }
        }
...
// Cuando PasarArticulo() llena tb_articulo, tb_cod, etc., disparamos el change
$(document).on('click', '[onclick*="PasarArticulo"]', function () {
    setTimeout(function () { $('#tb_cod').trigger('change'); }, 100);
});
</script>
