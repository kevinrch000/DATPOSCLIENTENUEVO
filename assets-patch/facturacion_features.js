(function ($, window) {
    ...
    function _fixImagenes() {
        $('#div_articulos article img, #div_articulos img').each(function () {
            ...
            if (esVacio) { $(this).attr('src', PLACEHOLDER); }
        });
    }
    ...
    function _flattenArticulosGrid() {
        var $container = $('#div_articulos');
        $container.find('.row .col-md-3 > div[id], .row .col-md-3 > div[onclick]')
            .each(function () { $container.append($(this)); });
