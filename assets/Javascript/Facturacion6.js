...
function whatsapp() {
    // Función legacy: ahora se llama desde FinalizarResumenDoc()
    // Mantener vacía para compatibilidad
}

/**
 * Enviar factura por WhatsApp usando la API del backend.
 * Abre una ventana wa.me con el mensaje formateado.
 */
function enviarWhatsApp(callback) {
    var numero = $('#txtnumwhatsapp').val().trim();
    if (numero.length < 9) {
        alertify.error('Número de WhatsApp inválido (mínimo 9 dígitos)');
    ...
    $.ajax({
        type: 'POST',
        url: basePath + '/api/whatsapp/enviar_factura.php',
        contentType: 'application/json; charset=utf-8',
        dataType: 'json',
        data: JSON.stringify({
            numero: numero,
            num_documento: $('#lb_doc').text().trim(),
            monto_total: $('#lb_total').text().trim(),
            nombre_empresa: ($('#hhd_empresa').val() || 'DatPOS').trim(),
            ruc_empresa: ($('#hdd_ruc').val() || '').trim(),
            fecha_documento: $('#fecha_emision_documento').val() || ''
        }),
        success: function(response) {
            if (response.success && response.url) {
                window.open(response.url, '_blank');
                alertify.success('WhatsApp abierto correctamente');
            } else {
                alertify.error('Error WhatsApp: ' + (response.error || 'Error desconocido'));
            }
            if (callback) callback();
        },
        error: function(xhr, status, error) {
            console.error('Error WhatsApp:', error);
            // Fallback: abrir wa.me directamente sin el backend
            var numCompleto = numero.length === 9 ? '51' + numero : numero;
            var texto = 'Comprobante de pago: ' + $('#lb_doc').text().trim() + 
                        ' - Total: ' + $('#lb_total').text().trim();
            window.open('https://wa.me/' + numCompleto + '?text=' + encodeURIComponent(texto), '_blank');
            alertify.warning('WhatsApp abierto (modo básico)');
            if (callback) callback();
        }
    });
}
...
/**
 * Generar PDF del ticket como base64 string usando html2canvas + jsPDF.
 * @param {function} callback - function(pdfBase64String)
 */
function generarPDFBase64(callback) {
    var $ticket = document.querySelector('#zona-imprimir');
    if (!$ticket) {
        console.error('No se encontró #zona-imprimir');
        if (callback) callback(null);
        return;
    }

    // Asegurar que el ticket sea visible para la captura
    var wasHidden = ($('#zona-imprimir').css('display') === 'none');
    if (wasHidden) $('#zona-imprimir').show();
    ...
    html2canvas($ticket, { scale: 2, useCORS: true }).then(function(canvas) {
        ...
        if (wasHidden) $('#zona-imprimir').hide();
...
/**
 * Enviar factura por Email usando la API del backend.
 * @param {function} callback - se llama al terminar (éxito o error)
 */
function enviarEmail(callback) {
    var email = $('#input_email_cliente').val().trim();
    ...
    generarPDFBase64(function(pdfBase64) {
        if (!pdfBase64) {
            alertify.error('Error al generar el PDF para email');
            if (callback) callback();
            return;
        }

        var numDoc = $('#lb_doc').text().trim();
        var basePath = window.DATPOS_BASE_PATH || '';

        $.ajax({
            type: 'POST',
            url: basePath + '/api/email/enviar_factura.php',
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            data: JSON.stringify({
                email: email,
                ...
                nombre_archivo: 'Factura_' + numDoc.replace(/\s+/g, '_') + '.pdf',
                monto_total: $('#lb_total').text().trim(),
                nombre_cliente: ($('#hdd_cdsc_coa').val() || 'Cliente').trim(),
                nombre_empresa: ($('#hhd_empresa').val() || 'DatPOS').trim()
            }),
            success: function(response) {
                if (response.success) {
                    alertify.success('📧 Email enviado correctamente a ' + email);
                } else {
                    alertify.error('Error email: ' + (response.error || 'Error desconocido'));
                }
                if (callback) callback();
            },
            error: function(xhr, status, error) {
                console.error('Error enviando email:', error);
                alertify.error('Error al enviar el email. Verifique la configuración SMTP.');
                if (callback) callback();
            }
        });
    });
...
//            return;
//        } else if ($('#txtfchDesde').val() == "") {fdocument).ready
...
//        $('#table_principal').DataTable().destroy();
        ...
        $.ajax({
            type: "POST",
            url: 'Facturacion.aspx/ConsultaDocumentosSunat',
...
function ClientePorDefecto(){
   ...
   var obj = llenarobjeto('Facturacion.aspx/ClientePorDefecto');
...
function PasarCuenta(obj){
    ...
    $.ajax({
        type: "POST",
        url: 'Facturacion.aspx/ConsultarCuentaDetalles',
...
function CargarCuentas(){
   ...
   var obj = llenarobjeto('Facturacion.aspx/ConsultarCuentas');
...
function GuardarCuenta(){
    ...
    $.ajax({
        type: "POST",
        url: 'Facturacion.aspx/GuardarCuenta',
        data: JSON.stringify({ cliente: $('#hdd_coa').val(), etiqueta: $('#tb_etiqueta').val(), detalle: objMovimientoDetalle }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
...
function CargarSubvariantes(){
    ...
    if($("#ddl_variante").val()!=""){
        ...
        $.ajax({
            type: "POST",
            url: 'Facturacion.aspx/ConsultarSubVariantesActivas',
...
function CargarVariantes(id_articulo){
    ...
    $.ajax({
        type: "POST",
        url: 'Facturacion.aspx/ConsultarVariantesActivas',
...
function Limpiar(){
    ...
    CompletarArticulosCategoria(llenarobjeto('Facturacion.aspx/CargarFavoritos'));
...
function Imprimir(){
    ImprimirComprobante("zona-imprimir");
}
...
function ArmarHtmlPrevio() {
    ...
    if (!obj || !obj[0]) { console.warn('ArmarHtmlPrevio: ConsultarTienda sin datos'); return; }
    ...
    if (objArticulos.length > 0) {
        ...
        $.each(objArticulos, function (index) {
            div_string = div_string + "<div class='col-xs-12' style='text-align: left; padding-left: 5px; font-weight: bold;'>" + objArticulos[index].cdsc_articulo + "</div>";
            div_string = div_string + 
            "<div style='text-align: center;'>"+
                "<div class='col-xs-4'></div>"+
                "<div class='col-xs-2' style='text-align: center; padding: 0;'>" + objArticulos[index].ncantidad + "</div>"+
                "<div class='col-xs-3' style='text-align: right; padding: 0;'>" + objArticulos[index].preciounitario + "</div>"+
                "<div class='col-xs-3' style='text-align: right; padding-right: 5px;'>" + objArticulos[index].nimporte_bruto + "</div>"+
            "</div>"
        });
...
function ArmarHtml() {
    ...
    var obj = llenarobjeto('Facturacion.aspx/ConsultarTienda');
    if (!obj || !obj[0]) { console.warn('ArmarHtml: ConsultarTienda sin datos'); return; }
    ...
    if($("#fecha_emision_documento").val()!='') {
        var dia = $("#fecha_emision_documento").val();
    } else {
...
function GenerarDocumento(){ 
    ...
    var obj = llenarobjeto('Facturacion.aspx/ConsultarTienda');
    if (!obj || !obj[0]) { console.warn('GenerarDocumento: ConsultarTienda sin datos'); return; }
...
//        descuento = parseFloat($('#hdd_precio').val()*descuento_ingresado/100);
        descuento = parseFloat($('#tb_precio').val()*descuento_ingresado/100);
//        cb_seleccionado = $('#lb_porcentaje')[0].innerText;
    ...
    if( descuento > parseFloat($("#hdd_descmax").val()))
    {
       ...
       $("#table_Articulos")[0].rows[$("#hdd_rv").val()].cells[0].innerHTML = "<div>"+$("#tb_nombre").val()+"</div>" +
        ...
        $("#table_Articulos")[0].rows[$("#hdd_rv").val()].cells[2].innerHTML = $("#tb_precio").val();
        $("#table_Articulos")[0].rows[$("#hdd_rv").val()].cells[7].innerHTML = $("#tb_precio").val() - ($("#tb_precio").val() / $('#impuesto_actual').val())
        ...
        var importe_linea = parseFloat(($("#tb_cantidad").val()*$("#tb_precio").val())-($("#tb_cantidad").val()*descuento)).toFixed(2);
        ...
        else
        {
            Swal.fire({
              ...
              if (result.value) {
                ...
                $('#fecha_emision_documento').val("");

                $.ajax({
                    type: "POST",
                    url: 'Facturacion.aspx/Cobrar',
                    ...
                    success: function (response) {
                        ...
                        if (response.d == "-1") MensajeFinSession();
                            ...
                            if (response.d[0] == true) {
                                ...
                                $('#fecha_emision_documento').val(response.d[3]);
...
function FinalizarResumenDoc(){
    ...
    var needEmail = $('#ckb_Correo').prop('checked') && $('#input_email_cliente').val().trim().length > 0;
    var needWhatsApp = $('#ckbwhatsapp').prop('checked') && $('#txtnumwhatsapp').val().trim().length > 0;
    var needPrint = $('#ckb_Imprimir').prop('checked');

    // Mostrar spinner si hay envíos pendientes
    if (needEmail || needWhatsApp) {
        $('#btn_confirmar_texto').hide();
        $('#btn_confirmar_spinner').show();
        $('#btn_confirmar_resumen').prop('disabled', true);
    }
    ...
    var pending = 0;
    if (needEmail) pending++;
    if (needWhatsApp) pending++;
    ...
    function finalizarYLimpiar() {
        // Restaurar botón
        $('#btn_confirmar_texto').show();
        $('#btn_confirmar_spinner').hide();
        $('#btn_confirmar_resumen').prop('disabled', false);
    ...
    // Si no hay envíos, cerrar directamente
    if (pending === 0) {
        finalizarYLimpiar();
        return;
    }
    ...
    // Enviar Email
    if (needEmail) {
        enviarEmail(onTaskDone);
    }

    // Enviar WhatsApp
    if (needWhatsApp) {
        enviarWhatsApp(onTaskDone);
    }
}
...
//    doc.save('sample-file.pdf'); 
    ...
    $.ajax({
        type: "POST",
        url: 'Facturacion.aspx/RegistrarPdf',
...
function BuscarArticulos() {
    var textoLen = $('#tb_articulo').val().length;

    // Unificado: buscar con 3+ caracteres (incluye códigos de barras de 15)
    if (textoLen >= 3) {
        $.ajax({
            type: "POST",
...
function CategoriaClick(obj) {
    ...
    $.ajax({
        type: "POST",
        url: 'Facturacion.aspx/ConsultarArticulosCategoria',
        data: '{id_familia: "' + obj.id + '" }',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
...
{
    var obj = llenarobjeto('Facturacion.aspx/ConsultarCategoriasDisponibles');

    if (obj.length > 0) {

//        var color1 = Math.floor((Math.random() * 256)) + "," + Math.floor((Math.random() * 256)) + "," + Math.floor((Math.random() * 256))
//        $("#div_favoritos").append($("<tr><td id='input_fav'   class='cuadrado btn-sq btn-primary' onclick='Favoritos(this)' style='margin-left: 10px;font-size: 11px;vertical-align: middle;text-align: center;'  >Favoritos</td></tr></tr><tr style='height: 5px;'></tr> "));
...
// PasarArticulo() — definida en facturacion_features.js (modal de cantidad rápida)
// La versión original que agregaba directo con cantidad=1 fue removida para evitar
// conflicto de doble definición.

function PasarArticuloCodigo(codigo) {

    $.ajax({
        type: "POST",
        url: 'Facturacion.aspx/ConsultarArticuloPrecioCodigo',
        data: '{codigo: "' + codigo + '" }',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
...
function MensajeTurno(){
    Swal.fire({
          ...
          if (result.value) { window.location.replace("../Interfaces/Home.php");}
	});
}
...
function ActualizarFavorito(id,bprefer){
    ...
    $.ajax({
        type: "POST",
        url: 'Facturacion.aspx/ActualizarFavorito',
        ...
        success: function (response) {
            if (response.d) {
                ...
                if($('#input_fav').attr('class')=="cuadrado btn btn-sq btn-primary sombreado") CompletarArticulosCategoria(llenarobjeto('Facturacion.aspx/CargarFavoritos'));;
            }
...
function ObtenerIGV(){
        var obj = llenarobjeto('Facturacion.aspx/ObtenerIGV');
        $('#impuesto_actual').val(obj[0].Impuesto);
}
...
var objLogo = llenarobjeto('../Interfaces/Home.aspx/CargarFotoUsuario');
    ...
    inicar_menu_nivel3('Facturación', '1_li_Ventas', '2_li_Ventas_Operaciones', '3_li_Facturacion', '0');
    CargarCategorias();
    $('#input_fav').addClass("sombreado");

    var obj = llenarobjeto('Facturacion.aspx/CargarFavoritos');

    CompletarArticulosCategoria(obj);
...
function CompletarArticulosCategoria(obj) {
    ...
    // Generar cada artículo como div plano (hijo directo de #div_articulos).
    // El CSS grid de #div_articulos se encarga del layout.
    // Ya NO se usan .row/.col-md-3 — esto elimina los bugs de HTML malformado
    // y la necesidad de _flattenArticulosGrid().
    $.each(obj, function (index) {
        var imgSrc = (obj[index].iimage && obj[index].iimage.length > 30)
                     ? "data:image/png;base64," + obj[index].iimage
                     : placeholder;

        var precioStyle = (obj[index].ctip_articulo === 'S')
                          ? " style='background-color: #3c9bd6;'"
                          : "";

        html += "<div oncontextmenu='clickfavoritos(this.id," + obj[index].bprefer + ")'"
             +  " id='" + obj[index].id_articulo + "'"
             +  " onclick='PasarArticulo(this.id)'>"
             +    "<article>"
             +      "<a class='precio' title=''" + precioStyle + ">" + obj[index].precio + "</a>"
             +      "<img src='" + imgSrc + "' alt=''/>" 
             +    "</article>"
             +    "<p class='cuadrado_desc'>" + obj[index].cdsc_articulo + "</p>"
             +  "</div>";
    });
...
//                    { data: 'ccod_coa' },
 ...
 function NuevoNC() {
    ...
    $.ajax({
        type: "POST",
        url: '../Ventas/Facturacion.aspx/BuscarNCIdCliente',
...
$(document).off('keyup.facturacion_suggest').on('keyup.facturacion_suggest', function(e) {
  if (e.keyCode === 27) {
    $("#sugerencias_clientes").html("");
    $("#sugerencias_clientes").hide();
  }
...
function CargarTabla(){
    ...
    $('#table_id').DataTable({
        ...
        ajax: {
                ...
                async: false,
                contentType: "application/json; charset=utf-8",
                url: 'Facturacion.aspx/ConsultarUsuarios',
                data: function (d) {
