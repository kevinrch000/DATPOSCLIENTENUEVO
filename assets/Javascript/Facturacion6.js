...
function whatsapp() {
    ...
    if ($('#txtnumwhatsapp').val() != "") { 
        if( $('#ckbwhatsapp').prop('checked') ) {
            var date = new Date(); 
            ...
            var doc = ($('#lb_doc').text()).substring(3,7);
            var serie = ($('#lb_doc').text()).substring(8,16);
            var fecha = '';
            if ($('#fecha_emision_documento').val() != ''){
                fecha = $('#fecha_emision_documento').val();
            } else {
                fecha = Dia  + "/" + mes + "/" + date.getFullYear();
            }
            window.open("https://wa.me/" + $('#txtnumwhatsapp').val() + "?text=Confirmación de pago. http://44.234.250.34:449/home?tb_ruc=" + $('#hdd_ruc').val() + $('input:radio[name=tipo]:checked').val() + doc + serie + fecha +$("#div_total")[0].innerText, '_blank');
        } 
    } 
}
