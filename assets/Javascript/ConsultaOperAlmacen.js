...
function PasaDatosCodUsuario() {
    var table = $('#tableVisibleConsulUsuarioMultiple').DataTable();
    var selectedUsers = [];
    table.$('input[name=cbx_usuario]:checked').each(function() {
        var row = $(this).closest('tr');
        var dataObj = table.row(row).data();
        var user = dataObj ? dataObj.ccod_usuario : row.find('td:eq(1)').text().trim();
        if (user) {
            selectedUsers.push(user.trim());
        }
    });
    $('#txtUsuario').val(selectedUsers.join(','));
}

function ModalConsultarUsuarios() {
    $('#tableVisibleConsulUsuarioMultiple').DataTable().destroy();
    $('#table_secundariaConsultarUsuario').DataTable().destroy();

    var obj = llenarobjeto('../Consultas/ConfigGeneral.aspx/CargarListaUsuario');

    $('#tableVisibleConsulUsuarioMultiple').DataTable({
        "pageLength": 5,
        data: obj,
        columns: [
            { 
                ...
                render: function (data, type, row) {
                    var currentVal = ($('#txtUsuario').val() || '').trim();
                    var selected = currentVal.split(',').map(s => s.trim());
                    var isChecked = selected.includes(row.ccod_usuario) ? 'checked' : '';
                    return '<input type="checkbox" name="cbx_usuario" value="' + row.ccod_usuario + '" ' + isChecked + '>';
                },
...
function PasaDatosCodCliente() {
    var table = $('#tableVisibleConsulClientesMultiple').DataTable();
    var selectedCodes = [];
    table.$('input[name=cbx_cliente]:checked').each(function() {
        var row = $(this).closest('tr');
        var dataObj = table.row(row).data();
        var code = dataObj ? dataObj.ccod_coa : row.find('td:eq(1)').text().trim();
        if (code) {
            selectedCodes.push(code.trim());
        }
    });
    $('#txtCliente').val(selectedCodes.join(','));
}
 
function ModalConsultarClientes() {
    $('#tableVisibleConsulClientesMultiple').DataTable().destroy();
    $('#table_secundariaConsultarCliente').DataTable().destroy(); 

    var obj = llenarobjeto('../Operaciones/Ingresos.aspx/ConsultarProveedor');
    $('#tableVisibleConsulClientesMultiple').DataTable({
        "pageLength": 5,
        data: obj,
        columns: [
            { 
                data: 'cbx',
                render: function (data, type, row) {
                    var currentVal = ($('#txtCliente').val() || '').trim();
                    var selected = currentVal.split(',').map(s => s.trim());
                    var isChecked = selected.includes(row.ccod_coa) ? 'checked' : '';
                    return '<input type="checkbox" name="cbx_cliente" value="' + row.ccod_coa + '" ' + isChecked + '>';
                },
                className: "dt-body-center"
            },
            { data: 'ccod_coa' },
