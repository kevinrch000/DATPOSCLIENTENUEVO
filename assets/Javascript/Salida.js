...
function tab_datosclick() {
    if($('#operacion').val() == '') {
        if($('#hdd_ultimafila').val() != '') {
            ...
            $.ajax({
                type: "POST",
                url: 'Salida.aspx/ConsultarSalida',
...
function CargarDetalles(id) {
    ...
    $.ajax({
        type: "POST",
        url: 'Salida.aspx/ConsultarInventarioDetalleSalida',
        data: '{id: "' + id + '" }',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,  
...
//    document.getElementById("tb_serie").innerHTML = obj[0].vserie; 
//    document.getElementById("tb_num").innerHTML = obj[0].nnumero; 
...
//    $("#tb_num").val(obj[0].nnumero);
    ...
    $("#ddl_almacen").prop('disabled', 'disabled');
}
...
//    }else 
    ...
    for (var i = 0; i < objInventarioDetalle.length; i++) { 
        if (objInventarioDetalle[i].state != '3' ){
            listCodArticulo = listCodArticulo+  objInventarioDetalle[i].ccod_articulo+";"+objInventarioDetalle[i].ncantidad+";"; 
        }
    }
 ...
 $.ajax({
        ...
        success: function (response) {
            ...
            if (response.d == "-1") MensajeFinSession();
                ...
                $('#tbLisArticuloErrorExport').DataTable().destroy();
                ...
                $('#tbLisArticuloErrorExport').DataTable({
                    data: obj,
                    columns: [
                            { data: 'cdsc_articulo' },
                            { data: 'ncantidad' },
...
function EditarFila() {
    ...
    var objArticuloCantaArti = [
     ...
     $.ajax({
        type: "POST",
        url: 'Salida.aspx/VerificarCantaArti',
...
function ModalArticulos() {
    ...
    $('#tableExelArticulos').DataTable().destroy();
//    var obj = llenarobjeto('Salida.aspx/ConsultarArticulos');

   
     $.ajax({
        type: "POST",
        url: 'Salida.aspx/ConsultarArticulosSalida',
...
//    }
    ...
    $.ajax({
        type: "POST",
        url: 'Salida.aspx/ConsultarAlmEmpActivos',
        data: '{tienda: "' + "%%%" + '" }',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
        ...
        success: function (response) {
            ...
            if (response.d) {
                $("#ddl_almacen").removeAttr("disabled");
...
function CargarNumerador() {
 if($('#ddl_almacen').val() != '') {
  $.ajax({
            type: "POST",
            url: 'Salida.aspx/ConsultarNumeradorSalida',
            data: '{almacen: "' + $('#ddl_almacen').val() + '" }',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function CargarTabla() {
    ...
    var obj = llenarobjeto('Salida.aspx/ConsultarSalidas');
...
//    $('#table_id tr > td:nth-child(1), #table_id tr > th:nth-child(1)').hide();
}
 ...
 $("#tb_cod").on('keyup', function (e) {
        if (e.key === 'Enter' || e.keyCode === 13) { 
            $.ajax({
                ...
                data: '{ccod_articulo: "' + $('#tb_cod').val() + '",ccod_alm: "' + $('#ddl_almacen').val() + '" }',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                async: false,
...
//                             $('#tb_articulo').val(response.d[0].ccod_articulo);
...
//    Funcion para generar exel
    $("#thTablaArticulos").contextMenu({
        menuSelector: "#contextMenu",
        menuSelected: function (invokedOn, selectedMenu) {
            //click para exporta a exel 
            var blob = new Blob([document.getElementById('tableArticulosExportExel').innerHTML], {
    ...
    $("#thTablaIngresos").contextMenu({
      ...
      $("#thtableDetalleExport").contextMenu({
        menuSelector: "#contextMenu",
        menuSelected: function (invokedOn, selectedMenu) {
            //click para exporta a exel 
            $('#tableDetalleExport').DataTable().destroy();
            var objDetVariantes = $('#tabla tr:has(td)').map(function(i, v) {
            var $td =  $('td', this);
                return {
                            ccod_articulo: $td.eq(2).text(),
            ...
            var blob = new Blob([document.getElementById('DivtableDetalleExport').innerHTML], {
                type: 'application/xml;charset=utf-8', encoding: 'utf-8'
            });
            saveAs(blob, "Reporte del " + hoy + ".xls"); 
        }
      ...
      $("#thtableListStock").contextMenu({
        menuSelector: "#contextMenu",
        menuSelected: function (invokedOn, selectedMenu) {
            //click para exporta a exel 
            var blob = new Blob([document.getElementById('DivtableListStock').innerHTML], {
                type: 'application/xml;charset=utf-8', encoding: 'utf-8'
            });
            saveAs(blob, "Reporte del " + hoy + ".xls"); 
        }
    ...
    inicar_menu_nivel3('Salidas directas', '1_li_Almacen', '2_li_Operaciones','3_li_Salida', '1');
    CargarTabla();
    CargarAlmacenes();
