...
function Eliminar(){
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        $.ajax({
            type: "POST",
            url: 'Familias.aspx/Eliminar',
            data: '{familia: "' + $('#tb_codigo').val() + '" }',
            ...
            success: function (response) {
                ...
                if(response.d=="-1") MensajeFinSession();
                    ...
                    }else if(obj[0].ccod_lin == '547'){
                        MensajeError('', "El código de familia (" + $('#tb_codigo').val() + ") no se puede eliminar porque se encuentra asignados a articulos.",'warning','Cancelar');
...
if(navigator.onLine) {
    ...
    if($('#tb_codigo').val().length != 5 && $('#tb_codigo').val() != "" ){
       Mensaje('Advertencia','Ingresar código de familia de 5 digitos.','warning');
        return;
    }else if ($('#tb_codigo').val() == "") {
        Mensaje('Advertencia','Ingresar código de familia.','warning');
         return;
    }else if ($('#tb_descripcion').val() == "") {
        Mensaje('Advertencia','Ingresar nombre de familia.','warning');
         return;
    }else if ($('#ddl_estado').val() == null) {
        Mensaje('Advertencia','Ingresar estado de familia.','warning');
         return;
    }

    var obj = [
        {
            "ccod_lin": $('#tb_codigo').val(),
            "cdsc_lin": $('#tb_descripcion').val(),
    ...
    $.ajax({
        ...
        data: JSON.stringify({ familia: obj, operacion: $('#operacion').val() }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
        ...
        success: function (response) {
            ...
            if (response.d == "-1") MensajeFinSession();
                ...
                }else if(obj[0].ccod_lin == '2627'){
                    MensajeError('', "El codigo de familia (" + $('#tb_codigo').val() + ") se encuentra registrado.",'warning','Cancelar');
...
function table_two_click(tbody) {
   ...
   $('#hdd_ultimafila').val(fila[1].innerText);
    ...
    if($('#hdd_numerofilas').val()>0) {
        $.ajax({
            type: "POST",
            url: 'Familias.aspx/ConsultarFamilia',
...
function CompletarCampos(obj){
    $("#tb_codigo").val(obj[0].ccod_lin);
    $("#tb_descripcion").val(obj[0].cdsc_lin);
     $("#send").val(obj[0].ccolor);
...
function CargarTabla() {
 ...
 $('#table_id').DataTable().destroy();
    var obj = llenarobjeto('Familias.aspx/ConsultarFamilias');
...
$(document).ready(function () {
  ...
  //    Funcion para generar exel
    ...
    inicar_menu_nivel3('Familias', '1_li_Almacen', '2_li_Tablas', '3_li_Familias','1');
    CargarTabla();
