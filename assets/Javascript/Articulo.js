...
function Nuevo() {
    ...
    $("#tb_codigo").prop("readonly", true); // Asegurar que el código de artículo sea de solo lectura
...
$(document).ready(function () {
    ...
    $('#ddl_familia').on('change', function() {
        if ($('#operacion').val() === 'nuevo') {
            var ccod_lin = $(this).val();
            if (ccod_lin !== '') {
                $.ajax({
                    type: "POST",
                    url: 'Articulos.aspx/GetNextCodigoArticulo',
                    data: JSON.stringify({ ccod_lin: ccod_lin }),
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    async: false,
                    success: function(response) {
                        if (response && response.d) {
                            $('#tb_codigo').val(response.d);
                        }
                    },
                    error: function() {
                        $('#tb_codigo').val(ccod_lin + '00001');
                    }
                });
            } else {
                $('#tb_codigo').val('');
