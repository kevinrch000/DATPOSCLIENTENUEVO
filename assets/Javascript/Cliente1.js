...
function BuscarDatosSunatPorCodigo() {
    ...
    $('#tb_codigo').data('ruc-bloqueado', false);
    $('#tb_codigo').data('ruc-validated-code', '');
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            // Si la API no encontró el RUC, mostrar mensaje, limpiar campos y bloquear.
            ...
            if (!encontrado && !obj.nombre_o_razon_social) {
                ...
                $('#tb_codigo').data('ruc-bloqueado', true);
                $('#tb_codigo').data('ruc-validated-code', doc);
                $('#tb_descripcion').val('');
                $('#tb_direccion').val('');
                return;
            }

            // Validar Estado y Condición según la matriz
            var est = (obj.estado || '').toString().trim().toUpperCase().replace(/\s+/g, ' ');
            var cond = (obj.condicion || '').toString().trim().toUpperCase().replace(/\s+/g, ' ');

            var rucBloqueado = false;
            var msgBloqueo = "";

            if (est === 'SUSPENSION TEMPORAL' || 
                est === 'BAJA PROVISIONAL' || 
                est === 'BAJA DEFINITIVA' || 
                est === 'BAJA PROVISIONAL DE OFICIO' || 
                est === 'BAJA DEFINITIVA DE OFICIO' ||
                est === 'BAJA DE OFICIO' ||
                est.indexOf('BAJA') >= 0 ||
                est.indexOf('SUSPENSION') >= 0 ||
                est.indexOf('SUSPENDID') >= 0) {
                rucBloqueado = true;
                msgBloqueo = "El RUC consultado se encuentra en estado: " + est + ". No se permite el registro.";
            }

            if (rucBloqueado) {
                Mensaje('Error', msgBloqueo, 'error');
                $('#tb_codigo').data('ruc-bloqueado', true);
                $('#tb_codigo').data('ruc-validated-code', doc);
                $('#tb_descripcion').val('');
                $('#tb_direccion').val('');
                if ($('#ddl_estado').length) $('#ddl_estado').val('0');
                return;
            }

            // Si pasa la validación, guardar que está validado y no bloqueado
            $('#tb_codigo').data('ruc-bloqueado', false);
            $('#tb_codigo').data('ruc-validated-code', doc);
            ...
            if ($('#ddl_estado').length && obj.estado) {
                var estOriginal = obj.estado.toString().toUpperCase();
                $('#ddl_estado').val(estOriginal.indexOf('ACTIV') >= 0 ? '1' : '0');
            }
            ...
            // Mostrar advertencia si es ACTIVO + NO HABIDO / NO HALLADO
            if (est === 'ACTIVO' && (cond === 'NO HABIDO' || cond === 'NO HALLADO')) {
                Mensaje('Advertencia', 'El contribuyente tiene estado ACTIVO pero condición ' + cond + '.', 'warning');
            }
        },
        error: function () {
                    ...
                    success: function (response) {
                        ...
                        if (response.d == "-1") MensajeFinSession();
                            ...
                            if (res === true || (res && res.success === true)) {
                                var msg = 'Se realizó la operación con éxito.';
                                var action = res.action || '';
                                if (action === 'ELIMINADO') {
                                    msg = 'Se eliminó el cliente físicamente de la base de datos.';
                                } else if (action === 'INACTIVADO') {
                                    msg = 'El cliente cuenta con movimientos en el sistema, por lo que fue inactivado/desactivado.';
                                }
                                Mensaje('Correcto', msg, 'success');
...
function Guardar() {
    ...
    if (navigator.onLine) {
        ...
        // Validar matriz RUC de SUNAT si es RUC (11 dígitos)
        if (/^\d{11}$/.test(codVal)) {
            var originalRuc = $('#tb_codigo').data('original-ruc');
            var isNew = ($('#operacion').val() === 'nuevo');
            if (isNew || codVal !== originalRuc) {
                if ($('#tb_codigo').data('ruc-validated-code') !== codVal) {
                    BuscarDatosSunatPorCodigo();
                }
                if ($('#tb_codigo').data('ruc-bloqueado') === true) {
                    // Ya se mostró el Mensaje de error correspondiente en BuscarDatosSunatPorCodigo()
                    return;
                }
            }
        }
...
function CompletarCampos(obj) {
    ...
    $("#tb_codigo").data('original-ruc', obj[0].ccod_coa);
