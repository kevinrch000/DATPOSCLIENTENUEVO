...
function FinalizarResumenDoc(){
    ...
    function finalizarYLimpiar() {
        ...
        if (needPrint) {
            ...
            // Recargar para limpiar si se envió correo
            window.location.href = (window.DATPOS_BASE_PATH||'') + "/pages/Ventas/NotaCredito.php";
        }
    }
...
function enviarEmail(callback) {
    ...
    generarPDFBase64(function(pdfBase64) {
        ...
        var numDoc = $('#lb_numdoc').text().trim();
        ...
        $.ajax({
            ...
            data: JSON.stringify({
                ...
                nombre_archivo: 'NotaCredito_' + numDoc.replace(/\s+/g, '_') + '.pdf',
                monto_total: $('#lb_importetotal').text().trim(),
                nombre_cliente: ($('#upNomCliente').text() || 'Cliente').trim(),
...
function ArmarHtml() {
    Swal.fire({
        title: 'Advertencia',
        text: 'Por favor, no cierre el navegador se esta procesando el documento.',
        icon: 'warning',
...
function ModalDocFac(row) {
   ...
   for (var i = 0; i < objfactura.length; i++) {
        ...
        if(row.id == objfactura[i].id_cbfact ){
            ...
            var cdocCobr = (objfactura[i].cdocCobr || '').trim();
            $('#CobranDocumentoFac').text(cdocCobr); 
            $('#upDocumentoCombranza').text(cdocCobr);  
            if (cdocCobr === '') {
                $('#btnIrComprobante').closest('a').addClass('disabled').css('pointer-events', 'none');
            } else {
                $('#btnIrComprobante').closest('a').removeClass('disabled').css('pointer-events', 'auto');
            }
...
$(document).ready(function () {
    ...
    // FIX BUG 3.2.3: habilitar botón Nuevo al cargar para que el usuario
    // pueda iniciar el flujo sin tener que buscar primero en la lista.
    $('#btn_p_nuevo').removeClass("botones_des").addClass("botones_hab");
    $('#btn_p_imprimir').removeClass("botones_des").addClass("botones_hab");
