...
function Ejecutar() {
    ...
    if (navigator.onLine) {
        ...
        // FIX BUG 2.3.3: los selects nunca son null; se compara contra "" (vacío real).
        // Familia y UniMedida tienen opción "Todos" (%%%); no se bloquean si está seleccionada.
        if (!$('#txtFamilia').val()) {
            Mensaje('Advertencia', 'Seleccionar familia.', 'warning');
            return;
        } else if (!$('#slUniMedida').val()) {
